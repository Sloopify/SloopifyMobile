...
class TextEditingCubit extends Cubit<TextEditingState> {
  ...
  void toggleCurrentTextItalic() {
    emit(state.copyWith(italic: !(state.textPropertiesForStory.italic??false)));
  }

  void toggleCurrentTextUnderline() {
    emit(state.copyWith(underline: !(state.textPropertiesForStory.underline??false)));
  }
  ...
  void updateSelectedPositionedText(String id){
    final currentElement= state.allTextAlignment.where((e)=> e.id==id).first;
